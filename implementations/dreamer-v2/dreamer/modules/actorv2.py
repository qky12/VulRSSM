...
class Actor(nn.Module):
    ...
    def forward(self, posterior, deterministic):
        ...
        if self.discrete_action_bool:
            ...
            entropy = torch.distributions.Independent(base_dist, 1).entropy()
            dist = torch.distributions.TransformedDistribution(base_dist, TanhTransform())
            dist = torch.distributions.Independent(dist, 1)
            action = dist.rsample()
            safe_action = action.clamp(-1 + 1e-6, 1 - 1e-6)  # only used for log_prob
