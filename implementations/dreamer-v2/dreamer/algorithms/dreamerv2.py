...
from dreamer.modules.actorv2 import Actor
...
class DreamerV2:
    def __init__(
        ...
        if self.discrete_action_bool:
            self.rou, self.eta = 1, 1e-3
        else:
            self.rou, self.eta = 0, 1e-4
        self.agent_update_times = 0
        self.tau = config.parameters.dreamer.agent.critic.tau
        self.critic_update_freq = config.parameters.dreamer.agent.critic.update_time
        ...
        self.target_critic = Critic(config).to(self.device)
    ...
    def _model_update(self, data, dynamic_info):
        reconstructed_observation_dist = self.decoder(
            dynamic_info.posteriors, dynamic_info.deterministics
        )
        ...
        kl_divergence_loss = torch.mean(
            self.config.kl_alpha * torch.distributions.kl.kl_divergence(torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=dynamic_info.posterior_logits.detach()), 1),
                                                                        torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=dynamic_info.prior_logits), 1)) +
            (1 - self.config.kl_alpha) * torch.distributions.kl.kl_divergence(torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=dynamic_info.posterior_logits), 1),
                                                                        torch.distributions.Independent(torch.distributions.OneHotCategorical(logits=dynamic_info.prior_logits.detach()), 1))
        )
        model_loss = (
    ...
    def behavior_learning(self, states, deterministics):
        ...
        for t in range(self.config.horizon_length):
            action, action_log_prob, action_entropy = self.actor(state, deterministic)
            ...
            self.behavior_learning_infos.append(
                priors=state, deterministics=deterministic, action_log_prob=action_log_prob, action_entropy=action_entropy
            )
    ...
    def _agent_update(self, behavior_learning_infos):
        ...
        if self.config.use_continue_flag:
            ...
            continues = self.config.discount * torch.ones_like(target_values)
        ...
        advantage = (lambda_values - values[:, :-1]).detach()
        log_prob_term = -self.rou * (behavior_learning_infos.action_log_prob[:, :-1]).unsqueeze(dim=-1) * advantage
        dynamics_term = -(1 - self.rou) * lambda_values
        entropy_term = -self.eta * (behavior_learning_infos.action_entropy[:, :-1]).unsqueeze(dim=-1)

        actor_loss = torch.mean(torch.sum(log_prob_term + dynamics_term + entropy_term, dim=1), dim=0)
        ...
        self.agent_update_times += 1
        if self.agent_update_times % self.critic_update_freq == 0:
    ...
    def environment_interaction(self, env, num_interaction_episodes, train=True):
        for epi in range(num_interaction_episodes):
            ...
            while not done:
                ...
                action, _, _ = self.actor(posterior, deterministic)
